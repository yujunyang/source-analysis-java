...
public class ResourceHandlerRegistration {
	 ...
	 */
//		而当调用 addResourceLocations() 时，ResourceHandlerRegistration 对象其实接受了一组资源路径,可能来自 classpath ,
//	也可能来自文件系统，然后它将这些路径记录下来。如下所示 :
	public ResourceHandlerRegistration addResourceLocations(String... resourceLocations) {
		this.locationValues.addAll(Arrays.asList(resourceLocations));
		return this;
	}
//		最后生成静态资源处理器 (resource handler) 时，会用到上面提到的这些URL pattern 和资源路径 resource locations。
//	当应用开发人员通过上面的 registry.addResourceHandler("/repo/**").addResourceLocations("file:/tmp/") 这种方式配置完静态资源映射关系之后，
//	这些关系会被 ResourceHandlerRegistry registry 对象记录下来。然后在应用启动过程中,Spring MVC 配置执行阶段，具体来讲，是 bean resourceHandlerMapping 实例化过程中
	/**
