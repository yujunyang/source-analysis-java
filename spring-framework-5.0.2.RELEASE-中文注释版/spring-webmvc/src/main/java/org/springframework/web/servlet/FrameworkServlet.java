...
//			(1)通过构造函数的注入进行初始化。
//					当进入 initWebAppIicationContext 函数后通过判断 this.webApplicationContext != null 后，便
//				可以确定this.webApplicationContext是否是通过构造函数来初始化的。可是有读者可能会有疑问，
//				在initServletBean函数中明明是把创建好的实例记录在了 this.webApplicationContext中：
//						this.webApplicationContexts = initWebApplicationContext();
//					何以判定这个参数是通过构造函数初始化，而不是通过一次的函数返回值初始化呢？如果存在这个问题，那么就是读者忽略一个问题了：
//				在Web中包含SpringWeb的核心逻辑的  DispatcherServlet只可以被声明为一次，在Spring中已经存在验证，所以这就确保了如果
//				this.webApplicationContext != null,则可以直接判定 this.webApplicationContext 已经通过构造函
//				数初始化。
//			（2）通过contextAttribute进行初始化。
//					通过在 web.xml 文件中配置的servlet参数 contextAttribute 来査找 ServletContext 中对应的属性,
//			默认为 WebApplicationContextClass.getName()+ ".ROOT",也就是在 ContextLoaderListener加载时会创建 WebApplicationContext 实例，
//			并将实例以 WebApplicationContextclass.getName() + ".ROOT”为key放入ServletContext中，当然读者可以重写初始化逻辑使用自己创建的
//			(3)重新创建 WebApplicationContext 实例。
///		2. configureAndRefreshWebApplicationContext(cwac)：
//				无论是通过构造函数注入还是单独创建，都免不了会调用configureAndRefreshWebApplicationContext方法来对已经创建的WebApplicationContext
//			实例进行配置及刷新
///		3.刷新
//			onRefresh是FrameworkServlet类中提供的模板方法，在其子类DispatcherServlet中进行了重写，主要用于刷新Spring在Web功能实现中所必须使用的全局变量。
			...
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						...
						//https://blog.csdn.net/thewindkee/article/details/95483639
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		//获取 servlet 的初始化參教 contextClass,如果没有配量默认为 XmlWebAppllcationContext.class
		...
		//在 ContextLoaderListener 加载的时候初蛤化的 WebApplicationContext 类型实例
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		//	无论调用方式如何变化，只要是使用AlicationContext所提供的功能最后都免不了使用公共父类AbstractApplicationContext提供的refresh()进行配置文件加载。
		//加载配置文件及整合parent到wac的初始化过程以及使用场景。
		wac.refresh();
	}
...
//
//			函数中已经开始了对请求的处理，虽然把细节转移到了 doService函数中实现，但是我们不难看出处理请求前后所做的准备与处理工作。
//		(1)	为了保证当前线程的LocaleContext以及RequestAttributes可以在当前请求后还能恢复，提取当前线程的两个属性。
//		(2)	根据当前inquest创建对应的LocaleContext和RequestAttributes,并绑定到当前线程。
