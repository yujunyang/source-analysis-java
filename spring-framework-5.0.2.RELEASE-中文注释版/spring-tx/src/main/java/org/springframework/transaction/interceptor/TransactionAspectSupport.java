...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	 ...
	 */
//		从上面的函数中，我们尝试整理下事务处理的脉络，在Spring中支持两种事务处理的方式,
//	分别是声明式事务处理与编程式事务处理，两者相对于开发人员来讲差别很大，但是对于Spring
//	中的实现来讲，大同小异。在invoke中我们也可以看到这两种方式的实现。考虑到对事务的应
//	用比声明式的事务处理使用起来方便，也相对流行些，我们就以此种方式进行分析。对于声明
//	式的事务处理主要有以下几个步馨。

//	(1) 获取事务的属性。
//		对于事务处理来说，最基础或者说最首要的工作便是获取事务属性了，这是支撑整个事务功能的基石，
//	如果没有事务属性，其他功能也无从谈起，在分析事务准备阶段时我们已经分析了事务属性提取的功能，大家应该有所了解。
//	(2) 加载配置中配置的 TransactionManager
//	(3) 不同的事务处理方式使用不同的逻辑。
//		对于声明式事务的处理与编程式事务的处理，第一点区别在于事务属性上，因为编程式的事务处理是不需要有事务属性的，
//	第二点区别就是在 TransactionManager 上， CallbackPreferringPlatformlTransactionManager 实现 PlatformTransactionManager 接口，
//	暴露出一方法用于执行事务处理中的回调。所以，这两种方式都可以用作事务处理方式的判断。
//	(4) 在目标方法执行前获取事务并收集事务信息。
//		事务信息与事务属性并不相同，也就是 Transactionlnfo 与 TransactionAttribute 并不相同，
//	Transactionlnfo 中包含 TransactionAttribute 值息，但是，除了 TransactionAttribute 外还有其他事务信息，
//	例如 PlatformTransactionManager 以及 Transactionstatus 相关信息。
//	(5)	执行目标方法。
//	(6)	一旦出现异常，尝试异常处理。并不是所有异常，Spring 都会将其回滚，默认只对 RuntimeException 回滚。
//	(7)	提交事务前的事务信息清除
//	(8)	提交事务。
	@Nullable
...
//			对于传入的 TransactionAttribute 类型的掺数 txAttr,当前的实际类型是 RuleBasedTransactionAttribute,是由获取事务属性时生成，主要用于数据承载，
//	而这里之所以使用 DelegatingTransactionAttribute 进行封装，当然是提供了更多的功能。
//	(2)	获取事务。
...
//	根据之前几个步骤获取的信息构建 TransactionInfo 并返回。
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		/// 3. 记录事务状态
		//		根据指定的属性与status  准备一个 TransactionInfo
	 ...
	 */
	//		当已经建立事务连接并完成了事务信息的提取后，我们需要将所有的事务信息统一记录在 TransactionInfo 类型的实例中，这个实例包含了目标方法开始前的所有状态信息，
	//	一旦事务执行失败，Spring 会通过TransactionInfo() 类型的实例中的信息来进行回滚等后续工作。
	 ...
	 */
//		之前已经完成了目标方法运行前的事务准备工作，而这些准备工作最大的目的无非是对于程序没有按照我们期待的那样进行，
//	也就是出现特定的错误，那么，当出现错误的时候，Spring 是怎么对数据进行恢复的呢？
	protected void completeTransactionAfterThrowing(@Nullable TransactionInfo txInfo, Throwable ex) {
		...
		if (txInfo != null && txInfo.getTransactionStatus() != null) {
			...
			///	1. 回滚条件
			/// 		这里判断是否回滚默认的依据，是抛出的异常是否是 RuntimeException 或者是 Error 的类型
