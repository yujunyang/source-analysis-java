...
//		当然.在 Spring 中每个复杂的功能实现，并不是一次完成的，而是会通过入口函数进行一个框架的搭建，初步构建完整的逻辑，而将实现细节分摊给不同的函数。
//	那么，让我们看看事务的准备工作都包括哪些。
/// （1） 获取事务
/// 		创建对应的事务实例，这里使用的是 DataSourceTransactionManager 中的 doGetTransaction方法，创建基于 JDBC 的事务实例。
...
//	（7） 将事务信息记录在当前线程中。
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		...
		if (isExistingTransaction(transaction)) {
			...
			/// 2. 处理已经存在的事务
			///   之前讲述了普通事务就細程,但是 Spring  中支持多种事务的传播规则，比如 PROPAGATION_NESTED、PROPAGATION_REQUIRES_NEW等，
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			try {
				...
				/// 构造 transaction，包括设置 ConnectionHolder、隔离级别、timeout，如果是新连接，绑定当前线程。
				/// 对于一些隔离级别、timeout 等功能的设置并不是由 Spring 来完成的，而是委托给底层的 数据库连接 去做的，而对于数据库连接的设置就是在 doBegin 函数中处理的。
...
//		（1）PRORAGTION_REQUIRES_NEW 表示当前方法必须在它自己的事务里运行，一个新的事务将被启动，而如果有一个事务正在运行的话，则在这个方法运行期间被挂起。
//	 而 Spring 中对于此种传播方式的处理与新事务建立最大的不同点在于使用 suspend 方法将原事务挂起。将信息挂起的目的当然是为了在当前事务执行完毕后在将原事务还原。
//		（2） PROPAGATION_NESTED 表示如果当前正有一个事务在运行中，则该方法应该运行在一个嵌套的事务中，被嵌套的事务可以独立于封装事务进行提交或者回滾，如果封装事务不存
//	 在，行为就像 PRORAGTION_REQUIRES_NEW 对于嵌入式事务的处理，Spring 中主要考虑了两种方式的处理。
//			a. Spring 中允许嵌入事务的时候，则首选设置保存点的方式作为异常处理的回滚。
//			b. 对于其他方式，比如 JTA 无法使用保存点的方式，那么处理方式与 PRORAGTION_REQUIRES_NEW 相同，而一旦出现异常，则由 Spring 的事务异常处理机制去完成后续操作。
...
///	将事务信息记录在当前线程中。
	 ...
	 * @see #rollback
	 */
//		在真正的数据提交之前，还需要做个判断。不知道大家还有没有印象，在我们分析事务异常处理规则的时候，当某个事务既没有保存点又不是新事物，
//	Spring对它的处理方式只是设置 一个回滚标识。这个回滾标识在这里就会派上用场了，主要的应用场景如下。
//		某个事务是另一个事务的嵌入事务，但是，这些事务又不在Spring的管理范围内，或者无法设置保存点，那么Spring会通过设置回滚标识的方式来禁止提交。
//	首先当某个嵌入事务发生回滚的时候会设置回滾标识，而等到外部事务提交时，一旦判断出当前事务流被设置了回滚标识，则由外部事务来统一进行整体事务的回滾。
//	所以，当事务没有被异常捕获的时候也并不意味着一定会执行提交的过程。
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		...
		///	如果在事务链中已经被标记混滚，那么不会尝试提交事务，直接回滚
	...
	//			a.	当亨务状态中有保存点信息的话便不会去提交亨务.
	//			c.	当亨务非新事务的时候也不会去执行提交亨务操作.
//			此条件主要考虑内嵌事务的情况，对于内嵌事务，在Spring中正常的处理方式是将内嵌事务开始之前设置保存点，
//	一旦内嵌事务出现异常便根据保存点信息进行回滚，但是如果没有出现异常，内嵌事务并不会单独提交，而是根据事务流由最外层事务负责提交，
...
//		如果程序流通过了事务的层层把关，最后顺利地进入了提交流程，那么同样，Spring会将事务提交的操作引导至底层数据库连接的API.进行事务提交。
	 ...
	 */
	/// 2. 回滚处理
	///			当然，一旦符合回滚条件，那么 Spring 就会将程序引导至回滾处理函数中。
	@Override
	 ...
	 */
//		同样，对于在 Spring 中的复杂的逻辑处理过程，在入口函数一般都会给出个整体的处理脉络，
//	而把实现细节委托给其他函数去执行。我们尝试总结下 Spring 中对于回滾处理的大致脉络如下。

//	(1) 	首先是自定义触发器的调用，包括在回滚前、完成回滾后的调用，当然完成回滚包括正常回滾与回滾过程中出现异常，
//	自定义的触发器会根据这些信息作进一步处理，而对于触发器的注册，常见是在回调过程中通过 TransactionSynchronizationManager 类中的静态方法
//	直接注册：
//			public static void registersynchronization(Transactionsynchronization synchronization)

//	(2)		除了触发监听函数外，就是真正的回滚逻辑处理了。
//		a. 当之前已经保存的事务信息中有保存点信息的时候，使用保存点信息进行回滚.常用于嵌入式事务，对于嵌入式的事务的处理，内嵌的事务异常并不会引起外部事务的回滚。
...
//			这里使用的是 JDBC 的方式进行数据库连接，那么 getSavepoint() 函数返回的是 JdbcTransactionObjectSupport,也就是说上面函数会调用 JdbcTransactionObjectSupport 中的
...
//		b. 		当之前已经保存的事务信息中的事务为新事物，那么直接回滚。常用于单独事务的处理。对于没有保存点的回滾，Spring 同样是使用底层数据库连接提供的 API 来操作的.由于我
//			们使用的是 DataSourceTransactionManager,那么 doRollback 函数会此类中的实现：
//					@Override
...
//					}
//
//	c.			当前事务信息中表明是存在事务的，又不属于以上两种情况，多数用于 JTA,只做回滚标识，等到提交的时候统一不提交.
	private void processRollback(DefaultTransactionStatus status, boolean unexpected) {
		try {
			...
			/// 清空记录的资源并挂起的资源恢复
//			对于回滚逻辑执行结束后，无论回滾是否成功，都必须要做的事情就是事务结束后的收尾工作。
			cleanupAfterCompletion(status);
		}
	}
...
//			(2)	如果当前事务是新的同步状态，需要将绑定到当前线程的事务值息清除。
//			(3)	如果是新事物需要做些清除资源的工作。
	private void cleanupAfterCompletion(DefaultTransactionStatus status) {
		/// 设置状态是对事务信息作完成标识，以避免重复调用。
		status.setCompleted();
		/// 如果当前事务是新的同步状态，需要将绑定到当前线程的事务值息清除
		if (status.isNewSynchronization()) {
