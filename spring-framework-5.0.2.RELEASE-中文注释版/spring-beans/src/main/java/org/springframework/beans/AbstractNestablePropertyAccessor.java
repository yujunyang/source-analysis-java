...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
	...
	// todo 需要分析 processKeyedProperty
	//实现属性依赖注入功能
	@SuppressWarnings("unchecked")
	private void processKeyedProperty(PropertyTokenHolder tokens, PropertyValue pv) {
		// 调用 属性的 getter 方法，获取属性的值
		...
		// 注入 list类型 的属性值
			...
			Object convertedValue = convertIfNecessary(tokens.canonicalName, oldValue, pv.getValue(),
					requiredType, ph.nested(tokens.keys.length));
			int size = list.size();
			// 如果 list 的长度大于 属性值 的长度，则多余的元素赋值为null
	...
	// todo 需要分析 processLocalProperty
	private void processLocalProperty(PropertyTokenHolder tokens, PropertyValue pv) {
		...
		try {
			...
			///todo  调用  BeanWrapperImpl 中 BeanPropertyHandler（BeanPropertyHandler extends PropertyHandler） setValue 方法注入属性值
