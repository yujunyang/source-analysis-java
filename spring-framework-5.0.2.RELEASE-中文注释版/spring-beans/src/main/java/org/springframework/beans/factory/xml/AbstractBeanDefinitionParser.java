...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	...
	//在这个函数中大部分的代码是用来处理解析后的AbstractBeanDefinition转化为BeanDefinitionHolder并注册的功能，而真正去做解析的事情
	//委托给了函数parseInternal，正式这句代码调用了我们自定义的解析函数，但是parseInternal中并不是直接调用自定义的doParse函数，而是进行
	//了一系列的数据准备，包括对beanClass、scope、lazyInit等属性的准备。
	@Override
	@Nullable
