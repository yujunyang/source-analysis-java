...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	@Override
	// 重写 getBeanDefinition 从 beanDefinitionMap 获取 BeanDefinition
	...
	///		ApplicationContext 实现的默认行为就是在启动时将所有单例 bean 提前进行实例化。提前实例化意味着作为初始化过程的一部分，ApplicationContext 实例会
	/// 创建并配置所有的单例 bean。通常情况下这是一件好事，因为这样在配置中的任何错误就会即刻被发现。而这个实例化的过程就是在 finishBeanFactoryInititalization
	/// 中完成的。
	@Override
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			...
			// Bean 不是抽象的，是单态模式的，且 lazy-init 属性配置为false
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				...
				if (isFactoryBean(beanName)) {
					...
					if (isEagerInit) {
						// 调用 getBean 方法，触发容器对 Bean实例化 和 依赖注入过程
						getBean(beanName);
					}
				}
				else {
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		///	处理注册已经注册的 beanNameBeanName 情况
		if (oldBeanDefinition != null) {
			// 如果对应的 BeanName 已经注册且在配置中配置了 bean不允许被覆盖，则跑出异常
			...
			///冻结 frozenBeanDefinitionNames
			this.frozenBeanDefinitionNames = null;
		}

		///	检查是否有同名的 BeanDefinition 已经在 IOC容器 中注册
		if (oldBeanDefinition != null || containsSingleton(beanName)) {
			// 重置所有已经注册过的 BeanDefinition对应 的缓存
	...
	public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
		...
		/// ObejctFactory 与 ObjectProvider 类注入的 特殊处理
		else if (ObjectFactory.class == descriptor.getDependencyType() ||
				ObjectProvider.class == descriptor.getDependencyType()) {
			return new DependencyObjectProvider(descriptor, requestingBeanName);
		}
		/// javaxInjectProviderClass 类注入特许处理
	...
	///或者是使用了自定义的解析器，但是对于集合类型来说并不在解析范围之内，所以再次对不同类型进行不同情况的处理，虽然对于不同类型处理方式
	///不一致，但是大致的思路还是很相似的，所以函数中只对数组类型进行了详细地注释。
	@Nullable
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			///返回值的构成：key=匹配的beanName,value=beanName对应的实例化化后的bean（通过getBean(beanName)返回）
