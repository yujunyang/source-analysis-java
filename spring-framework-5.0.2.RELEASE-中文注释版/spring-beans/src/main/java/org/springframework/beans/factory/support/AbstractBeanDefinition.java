...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	@Nullable
	/**
	 * bean的作用范围，对应<bean>属性scope
	 */
	private String scope = SCOPE_DEFAULT;

	/**
	 * 是否抽象，对应<bean>属性abstract
	 */
	...
	/**
	 * 用来表示一个bean的实例化依靠另外一个bean先实例化，对应<bean>属性depend-on
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * autowire-candidate属性设置为fasle，这样容器在查找自动装配对象时，将不考虑该bean，即它不会被考虑作为其他bean自动装配的候选者，
	 * 但是该bean本身还是可以使用自动装配来注入其他bean的。
	 */
	private boolean autowireCandidate = true;

	/**
	 * 自动装配时当出现多个bean候选者时，将作为首选者，对应<bean>属性primary
	 */
	...
	/**
	 * 对应<bean>属性factory-method
	 */
	@Nullable
	private String factoryMethodName;

	/**
	 * 记录构造函数注入属性，对应<bean>属性constructor-arg
	 */
	@Nullable
	 ...
	 * 定义这个bean的应用，
	 * 	APPLICATION:用户，
	 * 	INFRASTRUCTURE（基础设施）:完全内部使用，与用户无关，
	 * 	SUPPORT:某些复杂配置的一部分
	 * 	程序设置
	 */
	...
	// 	1. 验证 方法存在性
	//	2. 设计方法没有被覆盖		（一箭双雕）
	//		在 bean 实例化 的时候如果检测到存在 methodOverrides 属性，会动态地为当前 bean 生成 代理 并使用对应的 拦截器 为 bean 做增强处理。
	//  但是,这里要提到的是，对于方法的匹配来讲，如果一个类中存在若干个重载方法，那么，在函数调用以及增强的时候需要根据参数类型进行匹配，
	//  来最终确认当前调用的到底是哪个函数。但是，Spring将一部分匹配工作在这里完成了，如果当前类的方法只有一个，那么就设置重载该方法没有被重载，
	//  这样后续调用的时候便可以直接使用找到的方法，而不需要进行方法的参数匹配验证了，而且还可以提前对方法存在性进行验证，可谓一箭双雕。
