...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	// Resource 为 FileSystemResource || UrlResource || FileUrlResource
	@Override
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		// 通过 resourcesCurrentlyBeingLoaded属性 来记录已经加载的资源
		Set<EncodedResource> currentResources = this.resourcesCurrentlyBeingLoaded.get();
		if (currentResources == null) {
			currentResources = new HashSet<>(4);
			this.resourcesCurrentlyBeingLoaded.set(currentResources);
		}
		/// encodedResource 在当前线程中已经存在。
	 ...
	 */
	//	从给定 XML 文件中实际载入 Bean 定义资源的方法
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {
			// 生成 DOM 对象
			// 将 XML 文件转换为 DOM对象，解析过程由 documentLoader 实现
	...
	protected Document doLoadDocument(InputSource inputSource, Resource resource) throws Exception {
		// 	loadDocument 这个方法很好地应用了面向对象中 单一职责 的原则，将逻辑处理委托给 单一的类 进行处理，而这个逻辑处理类就是 BeanDefinitionDocumentReader。

		// 	EntityResolver: EntityReslover 的作用是项目本身就可以提供一个如何寻找 DTD 声明的方法，即由程序来实现寻找 DTD 声明的过程，比如我们将

		// DTD 文件放到项目中某处，在实现时直接将此文档读取并返回给 SAX 即可。这样就避免了通过网络来寻找相应的声明。
	...
	protected int getValidationModeForResource(Resource resource) {
		//		可以对 XMLBeanDefinitionReader 中的 setValidationMode 进行设置，
		// 否者使用自动检测的方式，而自动检测验证模式功能是在函数 detectValidationMode()  方法中实现的，在
		//  detectValidationMode 函数中又将自动检测验证模式的工作委托给 XmlValidationModeDetector
	...
	//	Spring Xml配置文件xsd和dtd头部详解:https://blog.csdn.net/win7system/article/details/91553401
	//	位置：/org/springframework/beans/factory/xml/spring-beans.dtd

	//	自动检测验证模式功能是在函数 detectValidationMode()  方法中实现的，
	//	在 detectValidationMode 函数中又将自动检测验证模式的工作委托给 XmlValidationModeDetector
	 ...
	 */
	/// 按照 Spring 的 Bean 语义要求将 Bean 定义资源解析并转换为 容器内部数据结构
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {

		///	使用 DefaultBeanDefinitionDocumentReader 实例化 BeanDefinitionDocumentReader
		///	得到 BeanDefinitionDocumentReader 来对 xml格式 的 BeanDefinition 解析
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();

		/// 获得容器中注册的 BeanDefinition 数量，即 beanDefinitionMap.size()。
	...
	protected BeanDefinitionDocumentReader createBeanDefinitionDocumentReader() {
		///	 documentReaderClass 为 DefaultBeanDefinitionDocumentReader.class;
