...
public class DefaultNamespaceHandlerResolver implements NamespaceHandlerResolver {
	...
	public NamespaceHandler resolve(String namespaceUri) {
		...
		if (handlerOrClassName == null) {
			...
			try {
				...
				// 基本上可以推断getHandlerMappings的主要功能就是把读取Spring.handlers配置文件并将将配置文件缓存在map中。
	...
	private Map<String, Object> getHandlerMappings() {
		...
		// 如果没有被缓存则开始进行缓存  double-check
		if (handlerMappings == null) {
			synchronized (this) {
				handlerMappings = this.handlerMappings;
				if (handlerMappings == null) {
					try {
						// this.handlerMappingLocation 在构造函数中已经被初始化为: META-INF/Spring.handlers
						Properties mappings =
						...
						// 将 Properties 格式文件合并到 Map 格式的  handlerMapings 中
