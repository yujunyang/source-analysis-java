...
public abstract class AbstractBeanDefinitionReader implements EnvironmentCapable, BeanDefinitionReader {
	...
	public int loadBeanDefinitions(String location, @Nullable Set<Resource> actualResources) throws BeanDefinitionStoreException {
		//	获取在 IoC容器  初始化过程中设置的资源加载器

		/// 	 父类 AbstractApplicationContext 继承 DefaultResourceLoader，因此，容器本身也是一个资源加载器
		///	 即使 beanDefinitionReader.setResourceLoader(this);
		ResourceLoader resourceLoader = getResourceLoader();
		if (resourceLoader == null) {
			throw new BeanDefinitionStoreException(
					"Cannot import bean definitions from location [" + location + "]: no ResourceLoader available");
		}
		/// 所有 ApplicationContext 已经实现了 ResourcePatternResolver 接口
		if (resourceLoader instanceof ResourcePatternResolver) {
			// Resource pattern matching available.
			try {
				//  将指定位置的 Bean 定义的资源文件解析为 Spring IOC 容器封装的资源
				// 加载多个指定位置的 Bean 定义资源文件

				/// 获取 resource 资源文件，在 AbstractXmlApplicationContext 中已经尝试过获取
				...
				///  这段是作甚用的????	去看 DefaultBeanDefinitionDocumentReader -> importBeanDefinitionResource
