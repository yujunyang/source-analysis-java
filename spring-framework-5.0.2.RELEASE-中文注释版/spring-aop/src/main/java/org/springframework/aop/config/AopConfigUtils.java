...
//		SmartInstantiationAwareBeanPostProcessor ,而  SmartlnstantiationAwareBeanPostProcessor 又继承
//		自 InstantiationAwareBeanPostProcessor,也就是说在 Spring 中，所有 bean 实例化时 Spring 都会
//		保证调用其 postProcessAfterlnitialization 方法，其实现是在父类 AbstractAutoProxyCreator 类中实现。
	...
	///实现了自动注册AnnotationAwareAspectJAutoProxyCreator类的功能，同时这里还涉及到了一个优先级的问题，如果已经存在了自动代理创建器，
	///而且存在的自动代理创建器与现在的不一致，那么需要根据优先级来判断到底需要使用哪个。
	@Nullable
	private static BeanDefinition registerOrEscalateApcAsRequired(Class<?> cls, BeanDefinitionRegistry registry,
		...
		if (registry.containsBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME)) {
			...
			//如果已经存在自动代理创建器并且与要创建的一致，那么无需再次创建
			return null;
		}
