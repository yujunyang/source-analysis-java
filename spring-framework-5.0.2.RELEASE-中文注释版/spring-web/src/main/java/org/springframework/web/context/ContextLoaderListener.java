...
//	在web.xml配置这个监听器，启动容器时，就会默认执行它实现的方法，使用ServletContextListener接口，开发者能够在客户端请求提供服务之前
//	向ServletContext中添加任意的对象。这个对象在ServletContext启动的时候被初始化，然后在ServletContext整个运行期间都是可见的。
//		毎一个Web应用都有一个ServletContext与之相关联。ServletContext对象在应用启动时被创建，在应用关闭的时候被销毁。
//	ServletContext在全局范围内有效，类似于应用中的一个全局变量。
//		在ServletContextListener中的核心逻辑便是初始化WebApplicationContext实例并存放至 ServletContext 中。 ????
//---------------------------------------------------------------------------------------------------------------------
//		正式分析代码前我们同样还是首先了解ServletContextListener的使用。
//			(1 )创建自定义 ServletContextListener
//			首先我们创建ServletContextListener,目标是在系统启动时添加自定义的属性，以便于在
//		全局范围内可以随时调用。系统启动的时候会调用ServletContextListener实现类的contextInitialized方法.所以需要在这个方法中实现我们的初始化逻辑。
