...
//
//		根据以上静态代码块的内容，我们推断在当前类ContextLoader同样目录下必定会存在属
//	性文件ContextLoader.properties,査看后果然存在，内容如下：
//	org.Springframework.web.context.WebApplicationContext-org.Springframework.web.context.support.XmlWebApplicationContext
//		综合以上代码分析，在初始化的过程中，程序首先会读取ContextLoader类的同目录下的属性文件ContextLoader.properties,并根据其中的配置提取将要实现WebApplicationContext接
//	口的实现类，并根据这个实现类通过反射的方式进行实例的创建。
	static {
	 ...
	 */
//		这里涉及了一个常用类WebApplicationContext：在Web应用中，我们会用到WebApplicationContext,
//	WebApplicationContext 承自 Applicationcontext,在 Applicationcontext 的基础上又追加了一些特定于Web的操作及属性，
//	非常类似于我们通过编程方式使用Spring时使用的ClassPathXmlApplicationContext类提供的功能。
//
//		initWebApplicationContext 函数主要是体现了创建 WebApplicationContext 实例的一个功能
//	架构，从函数中我们看到了初始化的大致步骤。
//		(1)	WebApplicationContext 存在性的验证。
//		在配置中只允许声明一次ServletContextListener,多次声明会扰乱Spring的执行逻辑，所以这里首先做的就是对此验证，
//	在Spring中如果创建WebApplicationContext实例会记录在 ServletContext 中以方便全局调用，而使用的 key 就是
//	WebApplicationContext.ROOT_WEB_ APPLICATION_CONTEXT_ATTRIBUTE,所以验证的方式就是査看ServletContext实例中是否有对应key的属性。
...
//		(4)映射当前的类加载器与创建的实例到全局变量currentContextPerThread中。
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		...
		try {
			...
			// Spring 类加载 https://blog.csdn.net/qq_22873427/article/details/77579435
