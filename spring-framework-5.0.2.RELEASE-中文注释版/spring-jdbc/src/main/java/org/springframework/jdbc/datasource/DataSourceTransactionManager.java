...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	 ...
	 */
	/// 	对于一些隔离级别、timeout 等功能的设置并不是由 Spring 来完成的，而是委托给底层的数据库连接去做的，而对于数据库连接的设置就是在
	/// doBegin 函数中处理的。
...
//			（1）尝试获取连接。
//		当然并不是每次都会获取新的连接，如果当前线程中的 connectionHolder 已经存在，则没
//	有必要再次获取，或者，对于事务同步表示设置为 true 的需要重新获取连接。
//			（2）设置隔离级别以及只读标识。
//		你是否有过这样的错觉？事务中的只读配置是Spring中做了一些处理呢？ Spring中确实是
//	针对只读操作做了一些处理，但是核心的实现是设置 connection 上的 readonly 属性。同样，对
//	于隔离级别的控制也是交由 connection 去控制的。
//			（3）更改默认的提交设置。
...
//			（6）将 connectionHolder 绑定到当前线程。
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			...
			/// 设置标记位，标识当前连接已经被事务激活。设置判断当前线程是否存在事务的依据
	...
	protected void doCleanupAfterCompletion(Object transaction) {
		...
		if (txObject.isNewConnectionHolder()) {
			...
			/// 如果当前事务时独立的新创建的事务，则在事务完成时释放数据库连接
