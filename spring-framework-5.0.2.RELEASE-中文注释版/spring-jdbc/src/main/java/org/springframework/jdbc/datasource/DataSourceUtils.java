...
//		在数据库连接方面，Spring 主要考虑的是关于事务方面的处理。基于事务处理的特殊性, Spring 需要保证线程中的数据库操作都是使用同一个事务连接。
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		if (conHolder != null && (conHolder.hasConnection() || conHolder.isSynchronizedWithTransaction())) {
			/// referenceCount 加 1
			conHolder.requested();
			/// 如果 connectionHandle == null
			if (!conHolder.hasConnection()) {
				logger.debug("Fetching resumed JDBC Connection from DataSource");
				///  设置一个 new SimpleConnectionHandle(connection)
		...
		/// 当前线程支持同步（TransactionSynchronizationManager 事务同步管理器）
	 ...
	 * @see #doGetConnection
	 */
//		数据库的连接释放并不是直接调用了 Connection 的 API 中的 close 方法。考虑到存在事务的情况，如果当前线程存在事务，
//	那么说明在当前线程中存在 共用数据库连接，这种情况下直接使用 ConnectionHolder 中的 released 方法进行连接数减一，而不是真正的释放连接。
