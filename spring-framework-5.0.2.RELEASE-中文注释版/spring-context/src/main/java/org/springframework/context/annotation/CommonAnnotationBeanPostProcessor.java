...
public class CommonAnnotationBeanPostProcessor extends InitDestroyAnnotationBeanPostProcessor
	...
	public PropertyValues postProcessPropertyValues(
		...
		// 获取 @Resource注 解中配置的 属性值 元数据
		InjectionMetadata metadata = findResourceMetadata(beanName, bean.getClass(), pvs);
		try {
			// 注入 属性值，与 AutowiredAnnotationBeanPostProcessor 中处理相同
	 ...
	 */
	// 根据给定 名称 或者 类型 获取 资源对象
	protected Object getResource(LookupElement element, @Nullable String requestingBeanName) throws BeansException {
		// 如果 注解对象 元素的 mappedName 属性不为空
		if (StringUtils.hasLength(element.mappedName)) {
			// 根据 JNDI名称 和 类型 去 Spring 的 JNDI容器中 获取 Bean
			return this.jndiFactory.getBean(element.mappedName, element.lookupType);
		}
		// 如果该 后置处理器 的 alwaysUseJndiLookup 属性值为 true
		if (this.alwaysUseJndiLookup) {
			// 从 Spring 的 JNDI容器中 查找指定 JDNI名称 和 类型的Bean
		...
		// 使用 autowiring 自动依赖注入 装配，通过给定的 名称 和 类型 从  资源容器 获取 Bean对象
	...
	protected Object autowireResource(BeanFactory factory, LookupElement element, @Nullable String requestingBeanName)
		...
		if (this.fallbackToDefaultTypeMatch && element.isDefaultName &&
			...
			// 根据容器中 Bean 定义解析给定的 依赖关系，将依赖以资源对象返回
		...
		// 根据 给定JDNI名称 和 类型 从 Spring 的 JDNI容器 查找资源
		else {
			resource = factory.getBean(name, element.lookupType);
			autowiredBeanNames = Collections.singleton(name);
		}

		// 为指定的 Bean 注册依赖的 Bean
