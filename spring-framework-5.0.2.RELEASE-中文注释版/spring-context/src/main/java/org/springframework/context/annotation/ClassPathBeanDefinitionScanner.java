...
//		scan是个全局方法，扫描工作通过doScan(basePackages透托给了 doScan方法，同时，还包括了 includeAnnotationConfig 属性的处理，
//	AnnotationConfigUtils.registerAnnotation ConfigProcessors (this.registry)代码主要是完成对于注解处理器的简单注册，
//	比如 AutowiredAnnotationBeanPostProcessor、RequiredAnnotationBeanPostProcessor ,这里不再黄述，我们重点研究文件扫描功能的实现。
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				if (candidate instanceof AnnotatedBeanDefinition) {
					...
					///	如果是 AnnotatedBeanDefinition 类型的 bean，需要检测以下常用注解：Primary、Lazy等
