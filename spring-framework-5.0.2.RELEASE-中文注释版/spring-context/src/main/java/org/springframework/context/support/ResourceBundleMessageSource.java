 ...
 */
/// 	HierarchicalMessageSource接口最重要的两个实现类ResourceBundleMessageSource和RealoadableResourceBundleMessageSource。
///	它们基于java的resourceBundle基础类实现，允许仅通过资源名加载国际化资源。ReloadResourceBundleMessageSource提供了定时刷新功能，
/// 允许在不启动系统的情况下，更新资源信息。StaticMessageSource主要用于程序测试，它允许通过编程的方式提供国际化信息。而DelegatingMessageSource
///是为方便操作父MessageSource而提供的代理类，仅仅举例ResourceBundleMessageSource的实现方式。
///		（1）定义资源文件
///			a.	messages.properties(默认：英文)，内容仅一句，如下：test=test
///			b.	messages_zh_CN.properties（简体中文）			test="测试"
///		然后cmd，打开命令行窗口，输入native2ascii -encoding gbk C:\messages_zh_CN.properties C:\message_zh_CN_tem.properties，
///		并将 C:\messages_zh_CN_tem.properties 中的内容替换到messages_zh_CN_tem.properties中，这样messages_zh_CN_tem.properties
///		文件就存放的是转码后的内容了，比较简单。
///
///		（2）定义配置文件
...
// 				</property>
// 			</bean>
///			其中，这个Bean的ID必须命名为messagaeSource,否则会抛出NoSuchMessageException异常。
///		（3）使用。通过ApplicationContext访问国际化信息。
///			String[] configs = {"applicaitonContext.xml"};
///			ApplicaitonContext ctx = new ClassXmlApplicationContext(configs);
///			//直接通过容器访问国际化信息
///			Obejct[] params = {"John",new GregorianCalendar().getTime()};
///			String str1= ctx.getMessage("test",params,Locale.US);
///			String str2 = ctx.getMessage("test",params,LOcale.CHINA)；
///			System.out.println(str1);
///			System.out.println(str2);
///		在initMessageSource中的方法主要功能是提取配置中定义的messageSource，并将其记录在Spring的容器中，也就是AbstractApplicationContext中。
/// 当然，如果用户未设置资源文件的话，Spring也提供合理默认的配置DelegatingMessageSource。
///
