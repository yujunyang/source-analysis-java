...
//      a. InitializingBean: 实现此接口的bean会在初始化时调用算 afterPropertiesSet 方法来进行bean的逻辑初始化.
//      b FactoryBean: 一旦某个bean实现此接口，那么通过 getBean 方法获取 bean 时其实是获取此类的 getObject() 返回的实例.
...
//      很显然，此函数主要目的就是对于 sqlSessionFactory 的初始化，通过之前展示的独立使用 MyBatis 的示例,
//    我们了解到 SqlSessionFactoiy 是所有 MyBatis 功能的基础。
   ...
   */
//    从函数中可以看到，尽管我们还是习惯于将 MyBatis 的配量与 Spring 的配置独立出来，
//  但是，这并不代表 Spring 中的配置不支持直接配置。也就是说，在上面提供的示例中，你完全
...
//      <property name="typeAliasesPackage" values="aaaaa"/>
//  </bean>

//    从这个函数中可以得知，配置文件还可以支持其他多种属性的配量，如 configLocation、objectFactory、 objectWrapperFactory、
//  typeAliasesPackage、typeAliases、typeHandlersPackage、plugins、 typeHandlers、transactionFactory、databaseldProvider、 mapperLocations
//      其实，如果只按照常用的配置，那么我们只需要在函数最开始按照如下方式处理 configuration:

//      xmlConfigBullder = new XMLConfigBuilder(this.configLocation.getInputStream(), null,this.configurationProperties);
//      configuration = xmlConfigBuilder.getConfiguration();

//    根据 configLocation 构造 XMLConfigBuilder 并进行解析，但是，为了体现Spring更强大的兼容性，Spring还整合了 MyBatis中其他属性的注人,
//  并通过实例configuratiaii来承载一步所获取的信息并最终使用 sqlSessionFactoryBuilder 实例根据解析到的 configuration 创建 SqlSessionFactory 实例。
  ...
  protected SqlSessionFactory buildSqlSessionFactory() throws Exception {
    ...
    /// 如果 objectFactory 存在 执行 targetConfiguration.setObjectFactory(objectFactory);方法
...
//      Arrays.stream(this.scriptingLanguageDrivers).forEach();
//      Stream.of(new HashMap<>()).forEach();
...
//    由于 SqlSessionFactoiyBean 实现了 FactoryBean 接口，所以当通过 getBean 方法获敢对应
//  实例时.其实是获取该类的 getObject() 函数返回的实例，也就是获取初始化后的 sqlSessionFactory 属性。
  /**
   * {@inheritDoc}
   */
  @Override
